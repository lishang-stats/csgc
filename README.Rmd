...
If we use the estimated labels to further get the MLE for $P$, we can calculate the centred subgraph count statistics and check if these statistics deviate from 0 or not.
...
If these csgc statistics deviate from 0, we can use our csgc greedy algorithm to adjust label estimations, and see if correct classification rate increases.
...
Finally, we make the csgc function compatible with other packages so that people can have easy access to these values. Before that, we will need the following packages: "ergm", "blockmodels" and "greed".
