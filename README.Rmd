...
Finally, we make the csgc function compatible with other packages so that people can have easy access to these values.
...
csgc(out$A, out$P)$t
```
For igraph object, we can easily extract adjacency matrix (A). If the graph follows a stochastic block model, we can estimate the communities size and labels, and estimate probability matrix (P) by maximising likelihood. 
...
csgc(out$A, out$P)$t
```
For fastRG package, we can extract adjacency matrix (A), and find the MLE of probability matrix (P) for SBM and DCSBM.
